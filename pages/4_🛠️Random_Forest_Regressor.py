...
st.markdown("""
    <h1 style='text-align: center;'>ML Algorithm:
        <span style='color: red;'> Random Forest Regressor Model</span>
        </h1>""",
...
st.markdown("---")
st.markdown("""
    <h1 style='text-align: center; color: green;'>Random Forest Regressor Model</h1>""",
    unsafe_allow_html=True
)
for i in range(3):
    st.text(" ")
...
for i in range(3):
    st.text(" ")
y_pred_continuous = rf_reg.predict(X_test)
threshold = 0.5
